...
export const STATE_FIELDS: readonly StateField[] = [
  ...
  },
  {
    ...
    states: ['MD', 'GA', 'KY', 'UT'],
    doc: [
      'Retirement income PER PERSON, because these states do not read it off a federal AGI and three of them cap an exclusion per person — so a return\'s totals do not determine its tax. Omit it and everything lands on one spouse, which is the worst of the cases, and the result says so in the name of the subtraction.',
      ...
      '  governmentPension — KY only: federal, Commonwealth or Kentucky local retired pay, military service included. The share attributable to service before 1 January 1998 is exempt WITHOUT LIMIT and does not consume the $31,110, so Kentucky has no maximum for that cohort.',
