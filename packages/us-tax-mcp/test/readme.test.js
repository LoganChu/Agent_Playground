...
test('README: the state coverage claims are the ones the engine actually holds', () => {
  ...
  assert.equal(taxing.length, 19);
  quotes('Eight of the nineteen taxing states cut their rate for 2026');

  const provisional = SUPPORTED_STATES.filter(
    (s) => getStateDefinition(s, 2026).status === 'provisional',
  );
  assert.equal(provisional.length, 6);
  quotes('six of the 2026 state-years carry at');
