...
| [`packages/us-tax-mcp`](packages/us-tax-mcp) | The same engine as an MCP server, so an AI assistant can compute tax rather than recall it. Six tools, zero dependencies, `npx -y us-tax-mcp`. |
...
Six tools: `estimate_federal_tax`, `compare_tax_years`, `effective_marginal_rate`,
`quarterly_estimated_payments`, `get_tax_parameters` and `list_supported_years`. The
middle two are the ones a bracket table cannot answer — 2025 was amended *retroactively*
after the IRS had published it, and the marginal rate on the next dollar is routinely
double the bracket once credit phase-outs are counted.
