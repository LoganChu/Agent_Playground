...
| Path | What it is |
| --- | --- |
| [`packages/us-federal-tax`](packages/us-federal-tax) | A zero-dependency US federal tax engine for JavaScript. Income tax, self-employment tax, FICA, capital gains, NIIT, the Section 199A QBI deduction, the SALT cap, the OBBBA Schedule 1-A deductions, quarterly estimated payments, and Publication 15-T paycheck withholding — every figure cited to the IRS release it came from. **v0.10.0 is a correction to a refundable credit**: a qualifying surviving spouse was taking the JOINT earned income credit phase-out threshold. § 32(b)(2)(B) raises it "in the case of a joint return" and a surviving spouse does not file one — § 2(a) hands them the joint RATE SCHEDULE and says nothing about § 32 — and the Revenue Procedure prints the grouping in its row heading: "Threshold Phaseout Amount (Single, Surviving Spouse, or Head of Household)". The error overstated the credit by up to `$1,161.75` (one child, `$45,000` of wages, 2026) for someone who had just lost a spouse, and it moved the STATE answer in six more states, because six of them set their own earned income credit as a flat percentage of the federal one. It is the one place in the package where that filing status does not follow the joint column, and it was found the first time a differential case used the status at all. |
...
npm i https://github.com/LoganChu/Agent_Playground/releases/download/us-tax-mcp-v0.26.0/us-tax-mcp-0.26.0.tgz
```
